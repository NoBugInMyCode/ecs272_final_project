...
def main():
    ap = argparse.ArgumentParser(description="Preprocess the Steam games CSV into cleaned CSVs for analysis.")
    ap.add_argument(
        "--input",
        default=None,
        help="Path to the input games CSV (default: data_processing/raw/games.csv)",
    )
    ap.add_argument(
        "--out",
        default=None,
        help="Path to output cleaned CSV (default: data_processing/steam_project_ready.csv)",
    )
    ap.add_argument(
        "--agg_out",
        default=None,
        help="Path to output aggregated CSV (default: data_processing/steam_project_agg.csv)",
    )
    ap.add_argument(
        "--sample_out",
        default=None,
        help="Path to output sample CSV (default: data_processing/steam_project_sample.csv)",
    )
    ...
    default_input = repo_root / "data_processing" / "raw" / "games.csv"
    default_json = repo_root / "data_processing" / "raw" / "games.json"
    args.input = Path(args.input) if args.input else default_input
    args.json_reviews = Path(args.json_reviews) if args.json_reviews else default_json

    if not args.input.exists():
        raise FileNotFoundError(f"Input file not found: {args.input}")

    if not args.json_reviews.exists():
        args.json_reviews = None

    args.out = Path(args.out) if args.out else repo_root / "data_processing" / "steam_project_ready.csv"
    args.agg_out = Path(args.agg_out) if args.agg_out else repo_root / "data_processing" / "steam_project_agg.csv"
    args.sample_out = Path(args.sample_out) if args.sample_out else repo_root / "data_processing" / "steam_project_sample.csv"

    for p in (args.out, args.agg_out, args.sample_out):
